#!/bin/bash
# ============================================================================
# Batch 3 Verification: Cache + Control
#  3.1 — Semantic cache hit
#  3.2 — Context compression trigger
#  3.3 — L1 autonomy (read-only suggestion)
#  3.4 — L2 autonomy (intervention required)
# ============================================================================

source "$(dirname "$0")/helpers.sh"

precheck_services

# ----- 3.1: Semantic cache hit ----------------------------------------------
scenario "3.1 — Semantic Cache Hit"

step "First query (cache miss)"
START1=$(date +%s%N)
RESP1=$(query_grpc "What is the capital of France?" "verify-user-3-1" "verify-ctx-3-1")
END1=$(date +%s%N)
ELAPSED1=$(( (END1 - START1) / 1000000 ))

verify "First query response received (no gRPC error)" \
    assert_not_contains "$RESP1" "error"

step "Second query — semantically similar (different wording)"
START2=$(date +%s%N)
RESP2=$(query_grpc "Tell me the capital city of France" "verify-user-3-1" "verify-ctx-3-1")
END2=$(date +%s%N)
ELAPSED2=$(( (END2 - START2) / 1000000 ))

if [ "$ELAPSED2" -lt "$ELAPSED1" ]; then
    verify "Second query faster than first (cache hit)" true
else
    verify_warn "Cache speedup not observed ($ELAPSED1 ms vs $ELAPSED2 ms)" false
fi

# ----- 3.2: Context compression trigger -------------------------------------
scenario "3.2 — Context Compression Trigger"

step "Send 15 long messages to trigger compression"
CTX="verify-ctx-3-2"
for i in $(seq 1 15); do
    query_grpc "This is a long conversation message number $i with substantial content to fill up the context window and eventually trigger the compression mechanism that should activate after about ten rounds of dialogue history." "verify-user-3-2" "$CTX" > /dev/null 2>&1 || true
done

step "Check trace_spans for compression marker"
verify "Trace spans contain context_compressed marker" \
    assert_pg_row_exists "trace_spans" "metadata::text LIKE '%context_compressed%'"

# ----- 3.3: L1 Autonomy — read-only suggestion ------------------------------
scenario "3.3 — L1 Autonomy — Read-Only Suggestion"

step "Send query with autonomy hint (via metadata field)"
RESP_L1=$(query_grpc "modify the config file to change the port" "verify-user-3-3" "verify-ctx-3-3" ', "metadata":{"autonomy_level":"1"}')

verify "L1 response is suggestion mode (not executing tool call)" \
    assert_not_contains "$RESP_L1" "tool_call_executed"

# ----- 3.4: L2 Autonomy — intervention required -----------------------------
scenario "3.4 — L2 Autonomy — Intervention Required"

step "Send query at L2 autonomy (via metadata)"
STREAM_OUT=$(query_stream_grpc "send an email to the team" "verify-user-3-4" "verify-ctx-3-4" ', "metadata":{"autonomy_level":"2"}')

verify "SSE stream contains event_type (L2 autonomy routing)" \
    assert_contains "$STREAM_OUT" "event_type"

step "Send PROCEED intervention response via UserExperienceService"
INT_RESP=$(send_grpc "agent_communication.UserExperienceService/InterventionResponse" '{"trace_id":"verify-ctx-3-4","decision":"PROCEED"}')

verify "InterventionResponse returns OK" \
    assert_contains "$INT_RESP" "OK"

# ----- Report ----------------------------------------------------------------
print_batch_report "Batch 3 — Cache + Control"
exit $(( FAIL_COUNT > 0 ? 1 : 0 ))

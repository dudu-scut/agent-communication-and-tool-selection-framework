...
# 安装 Consul (可选，当前项目使用自研 ServiceRegistry，基于内存/Redis，无需 Consul)
# 以下为预留接口，若未来迁移至 Consul 可参考：
...
REGISTRY_URL=http://localhost:8500  # 自研 ServiceRegistry（基于内存/Redis，非 Consul）
...
[registry]
url = http://localhost:8500  # 自研 ServiceRegistry（非 Consul）
...
### Registry 服务（自研 ServiceRegistry，基于内存/Redis）

创建 `/etc/systemd/system/agent-registry.service`:

```ini
[Unit]
Description=Agent Registry Server (自研ServiceRegistry)
...
> **注意**: 以下为未来规划方案，当前项目使用内置的可观测性方案（Redis + ObservabilityService），不依赖 Prometheus/Grafana。
...
> **注意**: 以下为未来规划方案，当前尚未实现 Prometheus metrics endpoint。
...
sudo ufw allow 8500/tcp   # Registry（自研 ServiceRegistry）
